...
export default function SurveyForm(_ref) {
    ...
    /** 유효성 검사 함수
      ...
      */
    var validates = function validates(formEl, length) {
        for (var i = 1; i <= length; i++) {
            var name = 'q' + i;
            var radioNodes = formEl[name];
            if (!radioNodes.value) {
    ...
    var handleSubmit = function handleSubmit(event) {
        ...
        if (!validates(formEl, QUESTION_STRINGS.length)) {
            return false;
        }
    ...
    return React.createElement(
        ...
        React.createElement(
            'ol',
            null,
            React.createElement(Surveys, { questions: QUESTION_STRINGS })
        ),
        React.createElement(
            'button',
