...
const QUESTION_STRINGS = [
    ...
    '어떤 일을 마치고 나면 주변 사람들이 부정적인 평가를 할까봐 걱정한다',
    ...
    return(
        <li className="survey" key={index}>
            ...
            <div className="inputs">
                ...
                <label>
                    <input type="radio" name={`q${index+1}`} value="3" defaultChecked />
                    <span>보통이다</span>
                </label>
                <label>
                ...
                <label>
                    <input type="radio" name={`q${index+1}`} value="5" />
                    <span>매우 그렇다</span>
                </label>
            </div>
        </li>
...
function handleSubmit(event) {
    ...
    const scores = getScores_(formData, EXCEPTION_NUMBERS, PART_NUMBERS)
    if (!scores) {
        return
    }
    showResult(scores)
